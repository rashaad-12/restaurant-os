...
**Do**
- **Do** put both layers on every write: a `@PreAuthorize` role gate **and**
  `ScopeGuard.assertWithinScope` in the service.
- **Do** call `ScopeGuard.assertCanView` on point reads, and scope list reads to the caller's
  `@RestaurantCodes`.
- **Do** derive the tenant (`restaurantCode`) from the token, never from a request-body field.
- **Do** keep MapStruct at every layer boundary.

**Don't**
- **Don't** expose a read/write path that touches menus without a scope check — even an internal
  helper (`getMenuByCodeAndRestaurantCode` is safe only because it's unexposed).
- **Don't** make a public diner-facing read reuse the staff-scoped `published` endpoint; a public
  read takes the restaurant code as input rather than from the token, and belongs on a separate route.
